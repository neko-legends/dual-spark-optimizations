...
class DeepSeekV4DSparkAttention(nn.Module):
    ...
    def _store_main_kv_ragged(
        ...
        """Scatter ragged mixed-prefill/decode rows into persistent KV slots."""
        ...
        if num_rejected_tokens is not None:
            rejected = num_rejected_tokens.to(
                device=main_x.device, dtype=torch.long, non_blocking=True
            ).view(batch_size)
        for i in range(batch_size):
            ...
            if rejected is not None:
                ...
                token_offsets = torch.arange(
                    seg_len, device=seg_kv.device, dtype=torch.long
                )
    ...
    def forward_dspark(
        ...
        *,
        ...
        main_kv_cache = (
            self.main_kv_cache
            if slot_index is None
            else self.main_kv_cache.index_select(0, slot_index)
        )
